...
export function downgradeTriggerActionsToHTML5() {
    let count = 0;
    // loop each trigger action that exists in alei
    for (let actionNum in special_values_table["trigger_type"]) {
    ...
    aleiLog(logLevel.INFO, `Deleted ${count} trigger actions`);
}